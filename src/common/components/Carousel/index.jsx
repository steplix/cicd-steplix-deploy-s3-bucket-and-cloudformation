          ...
          <div className="navigation-wrapper">
            <div ref={sliderRefMobile} className="keen-slider">
              {ARRAY_CLIENTS.map((element, index) => (
                <div key={`image-${index}`} className="keen-slider__slide number-slide">
                  <TestimonialCard
          ...
          <div className="navigation-wrapper">
            <div ref={sliderRef} className="keen-slider flex">
              {_.chunk(ARRAY_CLIENTS, 1).map((subimages, i) => (
                <div key={`subimage-${i}`} className="keen-slider__slide number-slide">
                  {subimages.map((element, j) => (
                    <div key={j}>
                      <TestimonialCard
                        ...
                        imageAlt={`${element.companyName} logo`}
                        clientFirstName={element.name}
                        clientPosition={element.position}
                        clientLastName={element.lastName}
                        clientOpinion={element.text}
                        clientCompany={element.companyName}
                        textColor="text-yellow"
            ...
            {loaded && propsRef.current && (
              <>
                <Arrow left onClick={e => e.stopPropagation() || propsRef.current?.prev()} disabled={currentSlide === 0} />

                <Arrow onClick={e => e.stopPropagation() || propsRef.current?.next()} disabled={currentSlide === propsRef.current.track.details.slides.length - 1} />
              </>
            )}
          </div>
        </div>
...
function Arrow(props) {
  return (
    <svg
      ...
      className={`h-8 w-8 absolute top-[50%] fill-[#fff] ${props.left ? 'left-10 lg:left-16 xl:left-1' : 'right-10 lg:right-16 xl:right-1'}`}
